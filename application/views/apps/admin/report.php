        ...
        <div class="col-lg-12 mt-2">
            <div class="card border-0 rounded-3 shadow-sm">
                <div class="card-header bg-white text-center">
                    <p class="card-title text-center h5 mt-2"><b>Laporan</b></p>
                </div>
                <div class="card-body">
...
<script>
    $(document).ready(function() {
        ...
        $('#records').DataTable({
            ...
            ajax: {
                url: "<?= base_url('admin/fetch_report') ?>",
                type: "POST",
                dataType: "json",
                dataSrc: ""
            },
            ...
            columns: [
                ...
                {data: "kd_meja",},
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        data.forEach(function(detail, index) {
                            ...
                            menuDetails += detail.nama_menu + '<span class="badge bg-dark">&#10799;' + detail.jumbel + '</span>,';
                        });
                        menuDetails = menuDetails.replace(/,\s*$/, "");
                ...
                {data:"responsible_person"},
                {
                    ...
                    render: function(data, type, row) {
                        let person = [];
                        data.forEach(function(detail, index) {
                            if (!person.includes(detail.head_chef)) {
                                person.push(detail.head_chef);
                            }
                        })
                        return person.join(",</br> ");
                    }
                },
                {
                    data:"menu_details",
                    render: function(data, type, row) {
                        let waiters = [];
                        data.forEach(function(detail, index) {
                            if (!waiters.includes(detail.delivery_person)) {
                                waiters.push(detail.delivery_person);
                            }
                        })
                        return waiters.join(",</br> ");
                    }
                },
                {
                    data: "tanggal_pesan",
